...
public class Negustor extends NPC{

    private Rectangle DetectionHeroRectangle;
    private boolean HeroDetected=false;

    public Negustor(RefLinks refLink, float x, float y) {
    ...
    public void Interact(){
        ...
        for(Element e : elements) {
            if(e.getId()==8 && e.getCount()>=20)
            {
                e.setCount(e.getCount()-20);
                active=false;
                if(LevelFlags.level2_part1_passed())
                    LevelFlags.level2_part2_coinsPaid=true;
                else
